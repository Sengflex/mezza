...
#include <mezza/collections/TokenSet.h>
...
TEST_IMPL_TEST(_tokenset_all) {
	...
	tokenset1 = TList_Create(TMemMgr_Init(&memmgr));
	TEST_CHECK_PREPARATION(tokenset1)

	res = TokenSet_Add(tokenset1, token1);
	...
	res = TokenSet_Add(tokenset1, token2);
	TEST_CHECK_PREPARATION(res)
	res = TokenSet_Add(tokenset1, token3);
	TEST_CHECK_PREPARATION(res)

	gottoken2 = TokenSet_Get(tokenset1, token2);
	...
	tokenset1 = TokenSet_CreateFromString(&memmgr, source1, sep1);
